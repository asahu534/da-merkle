...
function buildDynamicMediaImages(main) {
  main.querySelectorAll('a').forEach((a) => {
    ...
    // this DM anchor to a button; strip button classes before rebuilding.
...
/**
 * Applies Section Metadata: reads any `.section-metadata` block inside each
 * section, turns its `style` value(s) into section classes (e.g. `dark`),
 * stores other key/value pairs on the section dataset, then removes the block.
 * The vendored scripts/aem.js decorateSections does not do this, so we add it
 * here in the project-owned file.
 * @param {Element} main The main element
 */
function decorateSectionMetadata(main) {
  main.querySelectorAll(':scope > .section').forEach((section) => {
    const meta = section.querySelector(':scope > div > .section-metadata, :scope > .section-metadata');
    if (!meta) return;
    ...
    const wrapper = meta.closest('.section-metadata-wrapper') || meta.parentElement;
    meta.remove();
